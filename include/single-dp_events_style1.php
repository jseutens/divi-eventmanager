...
<section class="et_pb_module et_pb_fullwidth_header dem_detail_style1_fullwidth_header">               
    <div class="et_pb_fullwidth_header_container">
        <div class="header-content-container">
     		<h1 class="et_pb_module_header"><?php echo esc_attr ( get_the_title() ); ?></h1>
		</div>
    </div>
</section>
<?php 
while ( have_posts() ) : the_post(); 
    $dp_event_thumb = wp_get_attachment_image_src(get_post_thumbnail_id(), 'dem_detail1_1080_608');
    if( $dp_event_thumb[0] != ''){ 
			...
			<div class="et_pb_row">
				<div class="et_pb_column">
					<?php if( !empty ( $event_thumbnail_image_url ) ){ ?>
						<img src="<?php echo esc_url ( $event_thumbnail_image_url ); ?>" alt="<?php echo esc_attr ( get_the_title() ); ?>">
					 <?php } ?>
				</div> <!-- .et_pb_column -->    
			</div> <!-- .et_pb_row -->
		 ...
		 <div class="et_pb_row event_content">
			<div class="et_pb_column">
					<div class=" dem_detail_style1_event_content">
						<?php echo et_core_intentionally_unescaped( apply_filters( 'the_content', $event_content ), 'html' );  ?>
					</div>
			</div> <!-- .et_pb_column -->    
		</div> <!-- .et_pb_row -->
		<?php } ?>
		 <div class="et_pb_row event_info">
			<div class="et_pb_column <?php if ( $divi_dem_hide_form != 'on' ){ ?> et_pb_column_1_2 <?php }else{ ?> et_pb_column_4_4 <?php } ?>">
					...
					<div class="et_pb_text_inner">
						...
						<tbody>
							...
							<?php if( !empty( $event_venue ) ){  ?>
								<tr><td><i class="et-pb-icon">&#xe081;</i>&nbsp;&nbsp;<?php echo  esc_attr(rtrim($event_venue_address,', ')); ?></td></tr>
							<?php } 
							...
							if( !empty( $event_start_date ) ){
							   if( $event_start_date == $event_end_date ){  ?>
									<tr><td><i class="et-pb-icon">&#x7d;</i>&nbsp;&nbsp;<?php echo esc_html (date_i18n('F d, Y', esc_attr( $event_start_date ) )); ?></td> </tr>
							<?php  }else{ ?>
									<tr><td><i class="et-pb-icon">&#x7d;</i>&nbsp;&nbsp;<?php echo esc_html (date_i18n('F d, Y', esc_attr( $event_start_date ) )) .' '.esc_attr($dem_evt_to).' '. esc_html (date_i18n('F d, Y',esc_attr( $event_end_date ) ) ); ?></td></tr>
							<?php  }   
							} if( !empty( $event_start_date )  && !empty( $event_start_time )  && !empty( $event_end_time ) ){?>
									<tr><td><i class="et-pb-icon icon_calendar">&#xe023;</i>&nbsp;&nbsp;<?php echo esc_html (date_i18n('l', esc_attr( $event_start_date ) )); ?>, <?php echo esc_attr( $event_start_time ) .' '.esc_attr($dem_evt_to).' '; echo esc_attr( $event_end_time ) ; ?></td></tr>
							<?php } 
							if( !empty( $event_phone_number ) && $divi_dem_hide_tel_no_frm != 'on' ){ ?> <tr><td><i class="et-pb-icon ">&#xe090;</i>&nbsp;&nbsp;<?php echo esc_attr( $event_phone_number )?></td> </tr> <?php }
							if( !empty( $event_categories ) ){ ?> <tr><td><i class="et-pb-icon ">&#x6e;</i>&nbsp;&nbsp;<?php echo esc_attr( $event_categories )?></td></tr><?php }
							if( !empty( $event_tag_string ) ){?><tr> <td><i class="et-pb-icon ">&#xe018;</i>&nbsp;&nbsp;<?php echo esc_attr( $event_tag_string )?></td></tr><?php  }
							if( !empty( $event_website ) ){?> <tr><td><i class="et-pb-icon ">&#xe0e3;</i>&nbsp;&nbsp;<a href="<?php echo esc_url( $event_website ); ?>" target="_blank"><?php echo esc_url( $event_website );  ?></a></td></tr><?php } 
							if( !empty( $event_email_id ) ){ ?><tr><td><i class="et-pb-icon icon_mail">&#xe076;</i>&nbsp;&nbsp;<a href="mailto:<?php echo esc_attr( $event_email_id ); ?>"><?php echo esc_attr( $event_email_id ); ?></a></td></tr><?php } 
							if( !empty( $event_cost_name ) ){?>
							<tr>
								<td> 
									<?php  if($event_currency_prefix_suffix == 'suffix'){  ?>
							...
							<?php if( !empty( $event_noticket ) ){?>
								<tr><td><i class="et-pb-icon icon_ticket">&#xe014;</i>&nbsp;&nbsp;
								<?php 
									esc_attr_e( $divi_dem_evt_rm_tickets, 'dpevent' ); ?>: <strong><?php echo esc_attr( $event_noticket ); ?></strong></td> </tr>
							<?php } ?>
					 </tbody>
					 </table>
					 <table class="dem-detail-style1">
						<tbody>
								...
								<?php  if( !empty( $event_organizer_name ) ){ ?><tr> <td><i class="et-pb-icon icon_mail">&#xe08a;</i>&nbsp;&nbsp; <?php echo  esc_attr( $event_organizer_name ); ?></td> </tr><?php }
									    if( !empty( $event_email_id ) ){ ?> <tr><td><i class="et-pb-icon icon_mail">&#xe076;</i>&nbsp;&nbsp;<a href="mailto:<?php echo esc_attr( $event_email_id ); ?>"><?php echo esc_attr( $event_email_id ); ?></a></td></tr> <?php  }?>
						</tbody>
					</table>
					</div>
			...
			<div id="dem_detail_style1_contact_form">
					...
					<div class="et_pb_contact">
						<h6 class="dem_avilable_ticket">
							<?php 
								esc_attr_e( $divi_dem_evt_rm_tickets, 'dpevent' ); ?>: <strong><?php echo esc_attr( $event_noticket ); ?></strong>
						</h6>
					   ...
					   <form id="dem_ticket_inquiry" class=" clearfix" method="post" action="#" >
						 ...
						 <p class="et_pb_contact_field   et_pb_contact_field_half dem_ticket_booking_left" data-id="ticket_price" data-type="ticket_price">
							...
							<?php } ?>
								...
								<input name="return" value= "<?php echo esc_url ( $dem_paypal_return_url ); ?>" type = "hidden"> 
								<div class="dem_events_submit_button_container">
									<?php 
									if ( $dem_event_fully_booked == 'on' && $event_noticket == 0 ){
										 echo esc_html_e( $dem_evt_fully_booked_msg, 'dpevent' ); 
									}else{
...
<?php  if ( $divi_dem_display_event_location_section == 'Yes' ) { 
			if ( $google_map_value == "yes" ){ ?>
					<div class="et_pb_row dem_detail_style1_google_map">
						<div class="et_pb_column et_pb_column_4_4">
							<div class="et_pb_module et_pb_text">
								<div class="et_pb_text_inner">
									<h3 class= "dem_detail_style1_title">
										<?php
										esc_attr_e( $divi_dem_evt_location, 'dpevent' ); ?> 
									</h3>
								</div>
 ...
 <?php } }
 if ( $divi_dem_display_social_icon_section == 'Yes' ) {?>
        ...
        <div class="et_pb_column  ">
            <div class="et_pb_module et_pb_text">
                <div class="et_pb_text_inner">
                    <h3 class= "dem_detail_style1_title">
						<?php 
						esc_attr_e( $divi_dem_evt_share, 'dpevent' ); ?>
					</h3>
                </div>
...
if ( $divi_dem_display_event_gallery_section == 'Yes' ) {
global $post;
...
if( $dpevent_cust_imagebox != "0" ){ ?>
    <div class="et_pb_row image_gallery">
        <div class="et_pb_column et_pb_column_4_4">
            <div class="et_pb_module et_pb_text">
                <div class="et_pb_text_inner">
                    <h3 class="dem_detail_style1_image_title">
						<?php esc_attr_e( $divi_dem_evt_gallery, 'dpevent' ); ?>
					</h3>
                </div>
                    <div class="popup-gallery">
                        ...
                        <a href="<?php echo esc_url( $dpevent_cust_gallery_upload_image_src_large[0] ); ?>" title="<?php echo esc_attr ( get_the_title() ); ?>">
                            <img src="<?php echo esc_url( $dpevent_cust_gallery_upload_image_src[0] ); ?>" alt="<?php echo esc_attr ( get_the_title() ); ?>">
                        </a>
                        <?php endfor; ?>
                    </div>
            </div> <!-- .et_pb_text -->
